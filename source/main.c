...
int main() {
    ...
    if (fichier_existe("C:\\Users\\willi\\Desktop\\Programmation_C\\Projet_C_nv\\donnees\\sauvegarde.txt")) { // faudra changer le chemin d'accès
        printf("\n\033[33m J'ai detecté une sauvegarde. Veux tu la charger ? (Ecrire 1 si Oui ou 0 sinon) : \033[0m");
        int charger;
        scanf("%d", &charger);
    ...
    //on commence avec 2 tourelles de base
    tourelles = ajouter_tourelle(tourelles, 1,0, 5);
    tourelles = ajouter_tourelle(tourelles, 3,0, 3);
        ...
        if (tourelles_place<MAX_TOURELLES){
            ...
            while(i){
                ...
                if (cagnotte >= 100 ) { // Coût d'une tourelle : 100 et on est a moins de tourelles sur la grille que de la CONSTANTE MAX_TOURELLES defini plus haut juste en dessous des #include
                    tourelles = ajouter_tourelle(tourelles, l, col, 3); // Portée = 3 on pourra changé
                    cagnotte -= 100;
                ...
                if (choix==0 || choix==1){
                    ...
                    printf("Erreur ton choix n'est pas correct");
                    break;
                }
            }
        }
